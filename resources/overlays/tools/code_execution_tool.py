...
Pan Desktop Windows overlay: the local backend is transported over
``multiprocessing.connection`` which maps to AF_UNIX on POSIX and AF_PIPE
(Windows named pipes) on Windows. See ``tools/_ipc.py`` for the helpers.
...
# Pan Desktop Windows overlay helpers.  ``tools._ipc`` is shipped as a new
# file by the same overlay bundle — see resources/overlays/tools/_ipc.py.
from tools._ipc import ipc_family, make_ipc_address, new_authkey

_IS_WINDOWS = platform.system() == "Windows"
from typing import Any, Dict, List, Optional

# Availability gate: multiprocessing.connection works on every supported
# platform, so the sandbox is no longer Windows-restricted.
...
def check_sandbox_requirements() -> bool:
    """Code execution sandbox runs on every platform via multiprocessing.connection."""
...
# ---- multiprocessing.connection transport (local backend) ----------------
#
# Pan Desktop Windows overlay: replaced AF_UNIX socket transport with
# multiprocessing.connection, which transparently maps to AF_UNIX on POSIX
# and AF_PIPE (Windows named pipes) on Windows.  The authkey is taken from
# the HERMES_IPC_AUTHKEY env var so argv stays clean (argv is world-readable
# on both platforms, env is per-process).

_UDS_TRANSPORT_HEADER = '''\
"""Auto-generated Hermes tools RPC stubs."""
import binascii, json, os, shlex, time
...
def _connect():
    global _conn
    if _conn is None:
        address = os.environ["HERMES_IPC_ADDRESS"]
        family = os.environ.get("HERMES_IPC_FAMILY", "AF_UNIX")
        authkey_hex = os.environ.get("HERMES_IPC_AUTHKEY", "")
        authkey = binascii.unhexlify(authkey_hex) if authkey_hex else None
        _conn = Client(address, family=family, authkey=authkey)
        # Pan Desktop hardening (M1.1 code review): pop the IPC
        # credentials from os.environ AFTER the Client is established.
        # The sandboxed user script may spawn its own subprocesses
        # (subprocess.run, os.system, multiprocessing.Process, ...) and
        # every one inherits the parent env by default. Without this
        # pop, grandchild processes would see HERMES_IPC_AUTHKEY and
        # could mint their own connections back to the parent. The
        # _SECRET_SUBSTRINGS filter only runs at parent->child, not
        # child->grandchild. Popping here is the safest scope.
        os.environ.pop("HERMES_IPC_ADDRESS", None)
        os.environ.pop("HERMES_IPC_FAMILY", None)
        os.environ.pop("HERMES_IPC_AUTHKEY", None)
    return _conn
...
def _rpc_server_loop(
    ...
    Pan Desktop Windows overlay: this loop now uses
    ``multiprocessing.connection.Listener`` (AF_UNIX on POSIX, AF_PIPE on
    Windows) instead of a raw ``socket.socket``.  The wire format changes
    from newline-delimited JSON to Python object pickling via the
    ``Connection.send()``/``recv()`` helpers — which is safe here because
    both ends are Python in the same machine boundary and authkey-gated.
    """
    from model_tools import handle_function_call

    conn = None
    try:
        # Listener.accept() is blocking; wrap in a best-effort timeout
        # using the underlying socket when present (POSIX only — on
        # Windows named pipes, accept blocks until a client connects).
        try:
        ...
        while True:
            ...
            if not isinstance(request, dict):
                conn.send({"error": f"Invalid RPC request: expected dict, got {type(request).__name__}"})
                continue
        ...
        logger.debug("RPC listener timeout")
    except OSError as e:
        logger.debug("RPC listener error: %s", e, exc_info=True)
    finally:
        if conn is not None:
            try:
                conn.close()
...
def execute_code(
    ...
    # Pan Desktop Windows overlay: build a platform-appropriate IPC address
    # (AF_UNIX path on POSIX, \\.\pipe\... on Windows) and a fresh authkey
    # per sandbox invocation.  The child receives both via env vars — argv
    # is world-readable on every supported platform.
    ipc_address = make_ipc_address("hermes_rpc")
    ipc_family_name = ipc_family()
    ipc_authkey = new_authkey()

    tool_call_log: list = []
    tool_call_counter = [0]  # mutable so the RPC thread can increment
    exec_start = time.monotonic()
    listener: Optional[multiprocessing.connection.Listener] = None
    ...
    try:
        ...
        # --- Start RPC listener (AF_UNIX on POSIX, AF_PIPE on Windows) ---
        listener = multiprocessing.connection.Listener(
            ipc_address, family=ipc_family_name, authkey=ipc_authkey,
        )

        rpc_thread = threading.Thread(
            target=_rpc_server_loop,
            args=(
                listener, task_id, tool_call_log,
        ...
        for k, v in os.environ.items():
            ...
            if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
                child_env[k] = v
        # Pan Desktop Windows overlay: propagate the IPC address, family and
        # authkey via env vars rather than argv (argv is world-readable on
        # both POSIX and Windows; env is per-process).  The child stub
        # generated by _UDS_TRANSPORT_HEADER reads these back out of
        # os.environ when constructing its Client.
        child_env["HERMES_IPC_ADDRESS"] = str(ipc_address)
        child_env["HERMES_IPC_FAMILY"] = ipc_family_name
        child_env["HERMES_IPC_AUTHKEY"] = binascii.hexlify(ipc_authkey).decode("ascii")
        ...
        # Wait for RPC thread to finish.  Closing the listener breaks any
        # in-flight ``accept()`` and tears down the underlying socket/pipe
        ...
        if listener is not None:
            try:
                ...
                pass
        listener = None  # prevent double close in finally
        ...
        # Cleanup temp dir and IPC listener.
        if listener is not None:
            try:
                listener.close()
            except OSError as e:
                logger.debug("Listener close error: %s", e)
        import shutil
        shutil.rmtree(tmpdir, ignore_errors=True)
        # On POSIX the Listener removes the AF_UNIX socket file on close;
        # on Windows named pipes disappear automatically.  The defensive
        # unlink below covers the edge case where the Listener was never
        # successfully constructed (e.g. exception before the with block).
        if ipc_family_name == "AF_UNIX":
            try:
                os.unlink(ipc_address)
            except OSError:
