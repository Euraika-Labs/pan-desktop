...
class ToolContext:
    ...
    def upload_file(self, local_path: str, remote_path: str) -> Dict[str, Any]:
        ...
        if len(b64) <= chunk_size:
            ...
            # Pan Desktop Windows overlay: the temp path is a POSIX path
            # interpreted by the REMOTE sandbox's bash via self.terminal(),
            # NOT a host filesystem path. We must keep /tmp literally —
            # tempfile.gettempdir() would return a Windows path on a
            # Windows host, which the Linux sandbox can't open. Only the
            # PID suffix changes vs upstream, to avoid collisions between
            # concurrent uploads from different Hermes instances.
            tmp_b64 = f"/tmp/_hermes_upload_{os.getpid()}.b64"
